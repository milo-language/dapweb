// E2E for M9 co-debug: dapweb-mcp --attach joins a live dapweb-web session as a
...
// Needs dapweb-web on [port] targeting /tmp/dapweb_nested. Usage: bun tests/mcp-attach.ts [port]
...
// MCP co-debug peer
const mcp = spawn(["./dapweb", "mcp", "--attach", `localhost:${port}`, "--source", hello.sourcePath],
...
ok(init.result.serverInfo.name === "dapweb", "mcp initialize");
