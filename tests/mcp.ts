// E2E for `dapweb mcp`: drives the MCP stdio protocol like an AI client would.
// Usage: bun tests/mcp.ts [path-to-dapweb]
import { spawn } from "bun";

const bin = process.argv[2] ?? "./dapweb";
const proc = spawn([bin, "mcp", "--program", "/tmp/dapweb_demo", "--source", "/tmp/dapweb_demo.c"], {
...
ok(init.result.serverInfo.name === "dapweb", "initialize → serverInfo dapweb");
