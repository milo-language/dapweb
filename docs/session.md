...
> the editor — Run launches it. CLI: `dapweb <program> [args...]` implies
...
- **server** — one `dapweb web` process. Owns exactly one *session*.
...
- **peer** — one WS connection: browser tab or `dapweb mcp --attach`. N peers, identical state.
...
```
dapweb web UI → http://localhost:8080
  target: /tmp/dapweb_nested   (lldb)
  co-debug: dapweb mcp --attach localhost:8080

14:02:11 peer connected    browser  (1 peer)
14:02:15 run started       /tmp/dapweb_nested  pid 4242  adapter lldb-dap
...
`--verbose` adds them plus DAP request tracing for debugging dapweb itself. `--quiet` = banner
...
- **No invented schema.** Dapweb strips its own keys (below) + `type`/`request`/`name`, merges
  dialect launch defaults, then user keys win — the existing M8j passthrough, promoted from
  escape hatch to THE config.
- **Dapweb-only keys**, clearly separated, stripped before the DAP request:
...
- `hello` carries `history: [...]`; `historyChanged` broadcast on append. Any browser, any
  machine sees the same list. localStorage `dapweb.configHistory` retired (read-once
...
- `dapweb <program> [args...]` works: top-level positional = implicit
  `web --program …` with args; `type` inferred. The 90% invocation is `dapweb /tmp/a.out`.
...
2. §2 config canonicalization (launch.json shape, `type`/`request`/`name`, dapweb keys
...
7. §5 `dapweb <program>` positional + launch.json file support.
